	...
	public static class HttpURLCONNECTION extends AsyncTask<Void, Integer, Response> {
		...
		public HttpURLCONNECTION(){
			onProgressChangeListener = new OnProgressChangeListener() {				
				@Override
				public void onProgressChanged(int progress) {
					if(HttpURLCONNECTION.this.getStatus() == Status.RUNNING){
		...
		protected Response doInBackground(Void... params) {
			Response response;
			try {
				response = WebService.HttpUrlConnection(this.url, this.requestMethod,
						this.headers, this.parameters, onProgressChangeListener);
			} catch (Exception e) {
				response = new Response();
				response.setException(e);
			}
